...
class CollectionPage2 extends React.Component {
    ...
    componentDidMount(){
        this.db.collection('2').onSnapshot((snapShots)=>{
            this.setState({
...
getCol1=(id)=>{
    let docRef = this.db.collection('2').doc(id);
    docRef.get().then((doc)=>{
        if(doc.exists){
            this.setState({
